...
class APIComponent extends Object
{
  ...
  public function get_head_skin($username, $size = 50)
  {
    ...
    $width = imagesx($src);
    $size_head = ($width*8)/64;
    $coord_helm = ($width*40)/64;
    $dest = imagecreatetruecolor($size_head, $size_head);
    imagecopy($dest, $src, 0, 0, $size_head, $size_head, $size_head, $size_head);
    imagecopy($dest, $src, 0, 0, $coord_helm, $size_head, $size_head, $size_head);
    $final = imagecreatetruecolor($size, $size);
    imagecopyresized($final, $dest, 0, 0, 0, 0, $size, $size, $size_head, $size_head);
    imagepng($final);
    imagedestroy($dest);
    imagedestroy($final);
  }
